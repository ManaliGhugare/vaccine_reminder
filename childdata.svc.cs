...
namespace vacrem
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "childdata" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select childdata.svc or childdata.svc.cs at the Solution Explorer and start debugging.
    public class childdata : Ichilddata
    {
        string constr = ConfigurationManager.ConnectionStrings["vacrem"].ToString();

        public void DoWork()
        {
        }

        public List<child> childlist(string uid)
        {
            //StreamReader reader = new StreamReader(uid);
            //string xmlString = reader.ReadToEnd();
            //string returnValue = xmlString;

            List<child> chlst = new List<child>();

            SqlConnection con = new SqlConnection(constr);
            try
            {
                SqlCommand cmd = new SqlCommand("childlst", con);
